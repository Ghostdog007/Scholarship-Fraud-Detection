...
def _compute_mi_scores(columns, numeric_df, y, n_bins, sample_weights, classes):
    """
    Compute class-weighted Mutual Information for a list of columns.
    Class-weighting is needed because the 99.97% valid vs 0.03% fraud imbalance 
    makes unweighted MI give near-zero signal for fraud.
    """
    mi_scores = {}
    for col in columns:
...
def _pearson_prune(X_top, mi_series_candidates, threshold=0.90):
    """
    Prune highly correlated features to reduce redundancy before mRMR.
    0.90 is the project-tuned cutoff, per AGENTS.md Section 4.2 Step 3.
    """
    ...
    pruned_features = [f for f in X_top.columns if f not in to_drop]
    return pruned_features


def _run_mrmr(mi_series_candidates, X_pruned, max_features):
    """
    Run mRMR (minimum Redundancy Maximum Relevance) selection.
    mRMR is used over pure MI ranking because it penalizes redundancy 
    between selected features, preventing the model from seeing the same 
    signal through multiple correlated channels.
    """
    selected_features = []
...
def _deduplicate_protected(protected_present, mi_series, numeric_protected):
    """
    Deduplicate protected features to prevent exact copies from diluting importance.
    """
    ...
    return protected_final


def load_and_clean_data(filepath):
    df = pd.read_csv(filepath, low_memory=False)
    
    # 2.2 Drop 100% null columns (AGENTS.md Section 2.2)
    cols_to_drop = [
    ...
    # 2.3 Drop duplicate columns (AGENTS.md Section 2.3)
    dup_cols_to_drop = [
...
def engineer_features(df):
    ...
    # These features capture direct inconsistencies between application details, indicating potential fraud.
    ...
    # These features capture network-level anomalous behavior (e.g., multiple apps sharing the same phone/IP).
    ...
    # These features capture explicit violations of NIC policy rules (e.g., extreme age limits, income caps).
...
def main():
    ...
    candidate_mi = _compute_mi_scores(numeric_candidates.columns, numeric_candidates, y, n_bins, sample_weights, classes)
    protected_mi = _compute_mi_scores(numeric_protected.columns, numeric_protected, y, n_bins, sample_weights, classes)
    
    mi_scores = {**candidate_mi, **protected_mi}
    ...
    pruned_features = _pearson_prune(X_top, mi_series_candidates, threshold=0.90)
    X_pruned = X_top[pruned_features]
    
    # Step 4: mRMR selection
    print("mRMR selection...")
    selected_features = _run_mrmr(mi_series_candidates, X_pruned, MRMR_MAX_FEATURES)
            
    # Merge: mRMR candidates + all protected features
    protected_final = _deduplicate_protected(protected_present, mi_series, numeric_protected)
