...
applications (Postgres system of record; console CSV intake preserved)
  ...
  → 5-relation identity graph (shares_mobile/ip/father_name/mother_name/pincode),
    still built/stored in full — used unrestricted by XAI rings + dense-block
  → three detectors (ALL higher = more anomalous):
      hybrid_anomaly_score = feature_pred_error + LAMBDA_EDGE_SCORE·edge_pred_error
                              ...
                              RGCN encoder itself only trains/scores on
                              ENCODER_RELATION_IDS = mobile/ip/pincode since
                              2026-08-03 — father_name/mother_name excluded, see
                              Key Hyperparameters below)
...
| RGCN encoder relations | `ENCODER_RELATION_IDS=[0,1,4]` (shares_mobile/shares_ip/shares_pincode) since 2026-08-03 — `shares_father_name`(2)/`shares_mother_name`(3) excluded from what the Hybrid GraphMCM encoder trains/scores on (`_build_edge_index_and_types(..., relation_ids=ENCODER_RELATION_IDS)`), while `graph_builder_v3` still builds/stores the full 5-relation graph for XAI rings (`graph_viz_v3`) and `dense_block_detector_v3` (called unfiltered). Same diffuse-hub degree shape as the already-rejected dense-block pincode relation (mother_name mean degree 5.09, max 109, touching 35.7% of nodes vs mobile's 0.9%/max-5); no risk/normal discrimination in locked-checkpoint post-hoc relation ablation (~1.0x vs mobile's 19x, ip's 2.2x); 6-seed retrain-from-scratch ablation (2 cluster-density settings) never showed harm and consistently (3/3 higher-precision seeds) improved MOTHER_NAME_COLLISION PR-AUC. `N_EDGE_TYPES` stays 5 (architecture unchanged — relations 2/3 simply never populated for the encoder). Full record: `outputs/ablation/relation_ablation_result.json`, `src/ablation_relation_v3.py`. The NeighborLoader mini-batch path (`train_sampled`/`score_sampled`/`_make_neighbor_loader`/`_batch_inputs`, step-5 scale work, not yet in production use) closed this gap 2026-08-03 — both take a `relation_ids` param, default `ENCODER_RELATION_IDS`; `_make_neighbor_loader` zeroes fan-out (`[0,0]`) for excluded relations so NeighborLoader never materializes their neighbors, and `_batch_inputs` skips them again as a belt-and-suspenders check. Verified: sampled-batch `edge_type` values only ever take {0,1,4} across a full pass. Same fix also corrected `score_sampled`/`train_sampled`'s stale `fanout` default from `(25,10)` to the actually-adopted `(-1,-1)` exact-neighborhood setting (see next row) |
...
| RGCN relation ablation (XAI-only) | `hybrid_graphmcm_v3.compute_relation_ablation()`, added 2026-07-22 — RGCN has no learned attention (unlike the rejected HAN path), so this re-scores the locked checkpoint 5 extra times (one edge type masked out per pass, via `edge_type_tensor`) and reports which relation's removal improves feature-reconstruction fit most, per node. `outputs/relation_ablation_v3.csv`; narrated on XAI cards as "Neighbourhood-expectation driver"; never feeds fusion or a threshold. New pipeline step `relation_ablation` (`main_v3.py`, between `train_hybrid` and `subspace_if`). Since 2026-08-03 (`ENCODER_RELATION_IDS`), `shares_father_name`/`shares_mother_name` deltas are trivially ~0.000000 — those relations are never in the encoder's input to begin with, confirmed on the retrained live checkpoint |
...
~~NeighborLoader fan-out magnitude and shape~~ — **CLOSED (lead, 2026-07-21):
exact-neighborhood batching adopted, fanout `(-1,-1)`** (this list was stale —
corrected 2026-08-03; see `AGENTS.md` §7 item 2 for the ablation);
`pg_trgm` vs `difflib` equivalence; Postgres HA policy; batch cadence; 4th-seed confirmation of the
